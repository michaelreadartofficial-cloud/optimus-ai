              ...
              <div key={creator.id || i} className="flex items-center gap-4 bg-white border border-gray-200 rounded-2xl px-5 py-4 hover:shadow-sm hover:border-gray-300 transition-all cursor-pointer group" onClick={() => handleCreatorClick(creator)}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {creator.isVerified && <span className="text-blue-500 text-xs">Ã¢ÂÂ</span>}
                  </div>
...
const CreatorDetailPage = ({ creator, setPage, setPageState }) => {
  ...
  if (selectedVideo) {
    return (
      <div className="space-y-6">
        <button onClick={() => setSelectedVideo(null)} className="text-sm text-gray-400 hover:text-gray-900 font-medium flex items-center gap-1 transition-colors">ÃÂ¯ÃÂ¿ÃÂ½ Back to {creator.name}'s videos</button>
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-sm mt-1">{creator.name} ÃÂ¯ÃÂ¿ÃÂ½ {new Date(selectedVideo.publishedAt).toLocaleDateString()} ÃÂ¯ÃÂ¿ÃÂ½ {selectedVideo.durationFormatted}</p>
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => setPage("videos")} className="text-sm text-gray-400 hover:text-gray-900 font-medium flex items-center gap-1 transition-colors">ÃÂ¯ÃÂ¿ÃÂ½ Back to Discover</button>
